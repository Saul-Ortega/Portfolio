...
function Header() {
    ...
    return (
    <>
        <header className="bg-violet-600/60 border-black border-2 backdrop-blur-xs sticky top-1.5 lg:w-4xl lg:h-16 mx-auto rounded-2xl flex items-center z-999 sm:w-xl sm:h-24 xs:w-md xs:h-18">
            <nav className="w-full">
                <div className="lg:hidden xs:w-full xs:flex xs:flex-row xs:justify-end">
                    <button className="mr-15 hover:bg-violet-600/60 hover:rounded-2xl xs:cursor-pointer" onClick={toggleMenu}>
                        <MenuIcon open={open} />
                    </button>
                    {
                    open &&
                    <div className="hidden xs:bg-[#fdf6bf] xs:flex xs:justify-center xs:text-7xl xs:text-center xs:fixed xs:w-full xs:h-screen sm:mt-20 xs:mt-18 xs:border-black xs:border-2 xs:rounded-2xl xs:z-999">
