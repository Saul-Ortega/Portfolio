        ...
        <header className="bg-violet-600/60 border-black border-2 backdrop-blur-xs sticky top-1.5 lg:w-4xl lg:h-16 mx-auto rounded-2xl flex items-center z-999 sm:w-xl sm:h-24 2xs:w-md xs:h-18 xs:w-80">
            <nav className="w-full">
                <div className="lg:hidden xs:w-full xs:flex xs:flex-row xs:justify-end">
                    <button className="mr-15 xs:mr-7 hover:bg-violet-600/60 hover:rounded-2xl xs:cursor-pointer" aria-label="Menú" onClick={toggleMenu}>
                        <MenuIcon open={open} />
                    </button>
                    {
                    open &&
                    <div className="hidden xs:bg-[#fdf6bf] xs:flex xs:justify-center xs:text-5xl 2xs:text-6xl xs:text-center xs:fixed xs:w-full xs:h-[87.5dvh] sm:mt-20 xs:mt-18 xs:border-black xs:border-2 xs:rounded-2xl xs:z-999">
                        <ul className="xs:mt-2 xs:h-full xs:flex xs:flex-col xs:justify-evenly">
